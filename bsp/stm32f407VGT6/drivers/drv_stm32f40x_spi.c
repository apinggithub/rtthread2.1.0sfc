...
static rt_err_t configure(struct rt_spi_device* device, struct rt_spi_configuration* configuration)
{
    ...
    hspi.Init.TIMode = SPI_TIMODE_DISABLE;//SPI_TIMODE_ENABLE;
    hspi.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLED;
    hspi.Init.CRCPolynomial = 7;
 
    hspi.Instance = stm32_spi_bus->spiHandle.Instance;
    //hspi3 = hspi;
    /* init SPI */
    //HAL_SPI_DeInit(&hspi);
    HAL_SPI_Init(&hspi);
...
static rt_uint32_t xfer(struct rt_spi_device* device, struct rt_spi_message* message)
{
	...
	SPI_HandleTypeDef hspi = stm32_spi_bus->spiHandle;
                            ...
                            const char * spi_bus_name)
{
    ...
    {
        stm32_spi->spiHandle.Instance = SPI2;
#ifdef RT_USING_SPI_DMA
        /* Enable the DMA1 Clock */
        RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);
    ...
    {
    	stm32_spi->spiHandle.Instance = SPI3;
#ifdef RT_USING_SPI_DMA
	...
	hspix.Instance = stm32_spi->spiHandle.Instance;
